...
router.put('/', async (req, res) => {
  ...
  for (const s of settings) {
    ...
    await pool.query('UPDATE system_settings SET value = $1, updated_at = NOW() WHERE key = $2', [
      String(s.value),
      s.key,
    ])
  }
...
router.get('/stats', async (req, res) => {
  const [users, products, orders, revenue, recentOrders, ordersByStatus] = await Promise.all([
    ...
    pool.query('SELECT COALESCE(SUM(total), 0) AS total_revenue FROM orders WHERE status != $1', [
      'cancelled',
    ]),
    pool.query(
