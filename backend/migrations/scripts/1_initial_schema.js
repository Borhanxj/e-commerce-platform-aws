exports.up = (pgm) => {
  ...
  pgm.createTable({ schema: 'auth', name: 'customers' }, {
    customer_id:  { type: 'int', primaryKey: true, references: '"auth"."users"(id)', onDelete: 'CASCADE' },
    name:         { type: 'varchar(100)', notNull: true },
    tax_id:       { type: 'varchar(50)', notNull: true, unique: true },
    home_address: { type: 'text', notNull: true },
  });

  pgm.createTable({ schema: 'auth', name: 'sales_managers' }, {
    sales_manager_id: { type: 'int', primaryKey: true, references: '"auth"."users"(id)', onDelete: 'CASCADE' },
    name:             { type: 'varchar(100)', notNull: true },
  });

  pgm.createTable({ schema: 'auth', name: 'product_managers' }, {
    product_manager_id: { type: 'int', primaryKey: true, references: '"auth"."users"(id)', onDelete: 'CASCADE' },
    name:               { type: 'varchar(100)', notNull: true },
  });
};
