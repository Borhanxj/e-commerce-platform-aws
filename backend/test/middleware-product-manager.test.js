...
describe('requireProductManager middleware', () => {
  it('returns 403 when req.user is undefined', () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'Product manager access required' })
  ...
  it('returns 403 when role is sales_manager', () => {
  ...
  it('calls next() when role is product_manager', () => {
    const req = { user: { userId: 4, email: 'pm@example.com', role: 'product_manager' } }
    const res = makeRes()
    const next = jest.fn()
