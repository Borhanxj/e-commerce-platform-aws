...
const ROLE_LABELS = { customer: 'Customer', sales_manager: 'Sales Manager', product_manager: 'Product Manager', admin: 'Admin' }
...
function UserManagement({ token }) {
  ...
  async function handleCreate(formData) {
    const res = await fetch(`${API}/users`, { method: 'POST', headers, body: JSON.stringify(formData) })
    const data = await res.json()
  ...
  async function handleUpdate(userId, formData) {
    const res = await fetch(`${API}/users/${userId}`, { method: 'PUT', headers, body: JSON.stringify(formData) })
    const data = await res.json()
    ...
    <div className="um">
      <div className="um-toolbar">
        <form className="um-search-form" onSubmit={handleSearch}>
          ...
          />
          <select className="um-role-filter" value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value) }}>
      ...
      <div className="um-table-wrap">
        <table className="um-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="5" className="um-empty">Loading…</td></tr>
            ) : users.length === 0 ? (
              ...
              <tr key={u.id}>
                <td>{u.id}</td>
                <td>{u.email}</td>
                <td><span className={`um-role-badge um-role-${u.role}`}>{ROLE_LABELS[u.role]}</span></td>
                <td>{new Date(u.created_at).toLocaleDateString()}</td>
                <td className="um-actions">
                  <button className="um-btn um-btn-edit" onClick={() => setModal({ mode: 'edit', user: u })}>Edit</button>
                  <button className="um-btn um-btn-delete" onClick={() => setDeleteConfirm(u)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {deleteConfirm && (
        <div className="um-overlay" onClick={() => setDeleteConfirm(null)}>
          <div className="um-modal" onClick={(e) => e.stopPropagation()}>
            <h2>Delete User</h2>
            <p>Are you sure you want to delete <strong>{deleteConfirm.email}</strong>? This action cannot be undone.</p>
